  ...
  return (
    <section className="w-full">
      <div className="mx-auto pt-4 sm:px-6 lg:px-8 ">
        <div><img src={category.mainbanner} alt={category.name} className="w-full sm:h-[400px] bg-cover sm:bg-cover h-[250px]"/></div>
        <h2 className="text-2xl font-bold text-center my-4">
          {category.name} Collection
        </h2>
        <div>
        <div className="w-full m-4 mx-auto sm:m-0  ">
           <div className="mx-auto grid grid-cols-1 w-fit md:grid-cols-2 lg:grid-cols-3 gap-4 sm:mx-0 ">
          {category.child_categories.map((subcat) => (
            <div
              key={subcat.id}
              className="cursor-pointer w-72 sm:w-72 rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-shadow"
